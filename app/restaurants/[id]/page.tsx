    ...
    <div>
      ...
      <div className="mt-3 flex gap-4 overflow-x-scroll px-5 [&::-webkit-scrollbar]:hidden">
        {restaurant.categories.map((category) => (
          <div
            key={category.id}
            className="min-w-[167px] rounded-lg text-center text-[F4F4F4]"
          >
            <span className="to-muted-foreground text-xs">{category.name}</span>
          </div>
        ))}
      </div>

      <div className="mt-6 space-y-4 pl-5">
        {/* TODO: MOSTRAR A LISTA DE PRODUTOS MAIS VENDIDOS */}
      ...
      {restaurant.categories.map((category) => (
        <div className="mt-6 space-y-4 pl-5" key={category.id}>
